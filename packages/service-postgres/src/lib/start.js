...
const establishConnection = async (
  ...
  do {
    try {
      logVerbose(
        `[postgres]  Connection attempt ${attempts + 1}/${maxAttempts} to "${
          conn.name
        }"`,
      );
    ...
    (model) =>
      new Promise(async (resolve, reject) => {
        try {
          ...
          logError(
            `[postgres] failed to init model "${model.name}" in "${conn.name}" - ${err.message}`,
          );
          logDebug(err);
          reject({ model, err });
        }
  ...
  const promises = models.map(
    (model) =>
      new Promise(async (resolve, reject) => {
        ...
        try {
          logVerbose(
            `[postgres] start model "${model.name}" in "${conn.name}"`,
          );
          await model.start(
            conn.handler,
            getModel(model.name, conn.name),
            conn.hooksContext,
          );
          resolve();
        } catch (err) {
          logError(
            `[postgres] failed to start model "${model.name}" in "${conn.name}" - ${err.message}`,
          );
          logDebug(err);
          reject({ model, err });
        }
...
export default async ({
  ...
  try {
    ...
    throw new Error(
      `[postgres] onConnection hook failed for "${name}" - ${err.message}`,
    );
  }

  // Create custom schemas
  if (schemas) {
    const schemasP = schemas.map((schema) =>
      conn.handler.query(`CREATE SCHEMA IF NOT EXISTS ${schema};`),
    );
  ...
  try {
    ...
    throw new Error(
      `[postgres] init models failed for "${name}": ${err.model.name}, ${err.err.message}`,
    );
  }
  ...
  try {
    ...
    throw new Error(
      `[postgres] afterInitModels hook failed for "${name}" - ${err.message}`,
    );
  }
  ...
  try {
    ...
    throw new Error(
      `[postgres] start models failed for "${name}": ${err.model.name}, ${err.err.message}`,
    );
  }
  ...
  try {
    ...
    throw new Error(
      `[postgres] afterStartModels hook failed for "${name}" - ${err.message}`,
    );
  }
};
